...
class TransformationGizmo {
  ...
  Future<void> create({
    TransformationGizmoType type = TransformationGizmoType.translation,
  }) async {
  ...
  Future<void> _buildTranslationAxes() async {
    ...
    // Y Axis (Green, Up)
    final yGroup = await _createAxis(
      shaftGeom,
      headGeom,
      _greenMat!,
      Vector3(0, 1, 0),
    ...
    // X Axis (Red, Right)
    final xGroup = await _createAxis(
      shaftGeom,
      headGeom,
      _redMat!,
      Vector3(1, 0, 0),
    ...
    // Z Axis (Blue, Forward)
    final zGroup = await _createAxis(
      shaftGeom,
      headGeom,
      _blueMat!,
      Vector3(0, 0, 1),
  ...
  Future<ThermionEntity> _createRing(
    ...
    final ringMatrix = Matrix4.compose(
      Vector3.zero(),
      rotation,
      Vector3.all(1.0),
    );
  ...
  Future<void> update({Vector3? cameraPosition, Vector3? position}) async {
    ...
    final rootTransform = Matrix4.compose(
      targetPos,
      Quaternion.identity(),
      Vector3.all(scale),
    );
    ...
    for (final axis in [GizmoAxis.x, GizmoAxis.y, GizmoAxis.z]) {
      ...
      if (_type == TransformationGizmoType.rotation) {
        ...
        for (int i = 0; i < segments; i++) {
          ...
          final segDist = pointToSegmentDistance(
            mousePos,
            screenPoint1,
            screenPoint2,
          );
  ...
  Future<bool> startDrag(int screenX, int screenY) async {
    ...
    if (_type == TransformationGizmoType.rotation) {
      final startPos = await _getMarkerPositionOnRing(
        screenX,
        screenY,
        _activeAxis,
      );
  ...
  Future<void> _updateTranslationDrag(int screenX, int screenY) async {
    ...
    var gizmoScreenSpace = Vector2(
      ...
      viewport.height.toDouble() -
          (((gizmoNdc.y / 2) + 0.5) * viewport.height.toDouble()),
    );
    ...
    if (_attachedTarget != null && !_isDisposed) {
      ...
      // handling
      final localTransform = _worldToLocalTransform(
        _attachedTarget!,
        newWorldTransform,
      );
  ...
  Future<void> _updateRotationDrag(int screenX, int screenY) async {
    ...
    final startAngle = math.atan2(
      _dragStartScreen!.y - gizmoScreenSpace.y,
      _dragStartScreen!.x - gizmoScreenSpace.x,
    );
    final currentAngle = math.atan2(
      currentScreen.y - gizmoScreenSpace.y,
      currentScreen.x - gizmoScreenSpace.x,
    );

    // Update current marker position on the ring
    if (_currentMarker != null) {
      final currentPos = await _getMarkerPositionOnRing(
        screenX,
        screenY,
        _activeAxis,
      );
    ...
    final startRotation = Quaternion.fromRotation(
      _targetStartTransform!.getRotation(),
    );
  ...
  Future<void> _updateHighlights() async {
    ...
    for (final axis in [GizmoAxis.x, GizmoAxis.y, GizmoAxis.z]) {
      ...
      switch (axis) {
        case GizmoAxis.x:
          await _redMat?.setParameterFloat4(
            "baseColorFactor",
            1.0,
            0.0,
            0.0,
          ...
          break;
        case GizmoAxis.y:
          await _greenMat?.setParameterFloat4(
            "baseColorFactor",
            0.0,
            1.0,
            0.0,
          ...
          break;
        case GizmoAxis.z:
          await _blueMat?.setParameterFloat4(
            "baseColorFactor",
            0.0,
            0.0,
            1.0,
