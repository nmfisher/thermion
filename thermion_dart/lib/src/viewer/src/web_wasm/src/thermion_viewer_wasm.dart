...
///
/// An implementation of [ThermionViewer] that forwards calls to an Emscripten
/// JS module.
///
class ThermionViewerWasm implements ThermionViewer {
  final _logger = Logger("ThermionViewerWasm");

  static _EmscriptenModule? _module;
  static ThermionViewerWasm? _instance;
  ...
  ///
  ThermionViewerWasm._({JSObject? module, this.assetPathPrefix}) {
    if (module != null) {
  ...
  ///
  ///
  static Future<ThermionViewerWasm> create(
      {JSObject? module, String? assetPathPrefix}) async {
    if (_instance != null) {
      throw Exception("Only one instance can be created at any given time");
    }
    _instance =
        ThermionViewerWasm._(module: module, assetPathPrefix: assetPathPrefix);
  ...
  void _setAssetPathPrefix(String assetPathPrefix) {
    _module!.ccall(
        "ThermionWeb_setAssetPathPrefix",
        "void",
  ...
  Future _initialize() async {
    ...
    final context = _module!.ccall("ThermionWeb_createGlContext", "int",
        <JSString>[].toJS, <JSAny>[].toJS, null);

    final loader = _module!.ccall("ThermionWeb_getResourceLoaderWrapper",
        "void*", <JSString>[].toJS, <JSAny>[].toJS, null);

    _viewer = _module!.ccall(
        "Viewer_create",
        "void*",
        ["void*".toJS, "void*".toJS, "void*".toJS, "string".toJS].toJS,
        [context!, loader, null, null].toJS,
        null) as JSNumber;
    ...
    _sceneManager = _module!.ccall("Viewer_getSceneManager", "void*",
  ...
  Future<SwapChain> createHeadlessSwapChain(int width, int height) async {
    var swapchain = _module!.ccall(
        "Viewer_createHeadlessSwapChain",
        "void*",
        ["void*".toJS, "void*".toJS, "uint32_t".toJS, "uint32_t".toJS].toJS,
        [_viewer!, null, width.toJS, height.toJS].toJS,
        null) as JSNumber;
    return WasmSwapChain(swapchain.toDartInt);
  }

  ///
  ///
  ///
  Future<SwapChain> createSwapChain(int handle) async {
    throw Exception();
  }
  ...
  ///
  ///
  Future destroySwapChain(WasmSwapChain swapchain) async {
    if (_viewer == null) {
      return;
    }
    _module!.ccall(
        "Viewer_destroySwapChain",
        "void",
        ["void*".toJS, "void*".toJS].toJS,
        [_viewer!, swapchain.tSwapchain.toJS].toJS,
        null);
  }
  ...
  void _destroyViewer() {
    _module!.ccall(
        "Viewer_destroy", "void", ["void*".toJS].toJS, [_viewer].toJS, null);
  }
  ...
  Future<ThermionEntity> loadGlbFromBuffer(Uint8List data,
    ...
    final result = _module!.ccall(
        "SceneManager_loadGlbFromBuffer",
        "int",
        [
          "void*".toJS,
          "void*".toJS,
  ...
  Future<Uint8List> capture(
      {RenderTarget? renderTarget, SwapChain? swapChain, View? view}) async {
  ...
  Future setMorphAnimationData(
    ...
    for (int i = 0; i < meshNames.length; i++) {
      ...
      var dataPtr = _module!._malloc(frameData.data.length * 4);

      // Create a Float32List to copy the morph data to
      var dataList = td.Float32List.fromList(frameData.data);
  ...
  Future createGeometry(Geometry geometry,
    ...
    final entityId = _module!.ccall(
        ...
        [
          ...
          (materialInstance as ThermionWasmMaterialInstance?)?.pointer.toJS ??
              "".toJS,
        ].toJS,
        null) as JSNumber;
