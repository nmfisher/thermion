...
void main() async {
  ...
  capturePoses(ThermionViewer viewer, String assetName, List<double> fullWeights) async {
    Future<Float32List> captureWithWeights(ThermionAsset asset, List<double> weights) async {
      final morphSet = (await asset.getMorphTargetSets()).single;
      await morphSet.setAllWeights(weights);
      final pixels = (await testHelper.capture(viewer.view, null)).values.single;
      return Float32List.view(pixels.buffer, pixels.offsetInBytes, pixels.lengthInBytes ~/ 4);
    }

    final plain = await viewer.loadGltf("${testHelper.assetsDir}/$assetName");
    final plainRest = await captureWithWeights(plain, List.filled(fullWeights.length, 0.0));
    final plainFull = await captureWithWeights(plain, fullWeights);
    await viewer.destroyAsset(plain);

    final rebuilt = await viewer.loadGltf("${testHelper.assetsDir}/$assetName", rebuildVertices: true);
    final rebuiltRest = await captureWithWeights(rebuilt, List.filled(fullWeights.length, 0.0));
    final rebuiltFull = await captureWithWeights(rebuilt, fullWeights);
    await viewer.destroyAsset(rebuilt);

    return (plainRest: plainRest, plainFull: plainFull, rebuiltRest: rebuiltRest, rebuiltFull: rebuiltFull);
  }
  ...
  void expectPoseParity(
    ...
    expect(plainEffect, greaterThan(minEffect), reason: "$label: morph weights had no effect on the plain load");
    expect(rebuiltEffect, greaterThan(minEffect), reason: "$label: morph weights had no effect after rebuildVertices");

    // Identical weights must produce the same pose before and after the
    // rebuild (both at rest and fully morphed).
    expect(
      _meanAbsoluteDifference(plainRest, rebuiltRest),
      lessThan(parityEpsilon),
      reason: "$label: rest pose differs between plain and rebuilt loads",
    );
    expect(
      _meanAbsoluteDifference(plainFull, rebuiltFull),
      lessThan(parityEpsilon),
      reason: "$label: morphed pose differs between plain and rebuilt loads",
    );
  }
  ...
  test('dense morph deltas survive rebuildVertices (skinned cube)', () async {
    await testHelper.withViewer(
      (viewer) async {
        final poses = await capturePoses(viewer, "cube_with_morph_targets.glb", [1.0]);
        expectPoseParity("dense", poses.plainRest, poses.plainFull, poses.rebuiltRest, poses.rebuiltFull);
      },
      bg: kRed,
  ...
  test('sparse morph deltas survive rebuildVertices', () async {
    await testHelper.withViewer(
      (viewer) async {
        final poses = await capturePoses(viewer, "cube_with_morph_targets_sparse.glb", [1.0]);
        expectPoseParity("sparse", poses.plainRest, poses.plainFull, poses.rebuiltRest, poses.rebuiltFull);
      },
      bg: kRed,
      cameraPosition: Vector3(3, 2, 6),
    );
  ...
  test('multi-primitive morph targets survive rebuildVertices', () async {
    await testHelper.withViewer(
      (viewer) async {
        final poses = await capturePoses(viewer, "cube_with_morph_targets_two_prims.glb", [1.0, 0.0]);
        expectPoseParity(
          "multi-primitive (Grow)",
          poses.plainRest,
          poses.plainFull,
          poses.rebuiltRest,
          poses.rebuiltFull,
        ...
        final posesShift = await capturePoses(viewer, "cube_with_morph_targets_two_prims.glb", [0.0, 1.0]);
        expectPoseParity(
          "multi-primitive (Shift)",
          posesShift.plainRest,
          posesShift.plainFull,
          posesShift.rebuiltRest,
          posesShift.rebuiltFull,
        );
      },
      bg: kRed,
      cameraPosition: Vector3(0, 1.5, 5.5),
    );
  });

  test('MorphTargetSet names and counts are preserved by rebuildVertices', () async {
    await testHelper.withViewer(
      (viewer) async {
    ...
    await testHelper.withViewer(
      (viewer) async {
        ...
        for (final instanceIndex in [0, 1]) {
          ...
          final entity = childEntities.firstWhere(renderableManager.isRenderable);
          final morphTargets = await cube.getMorphTargets(entity);
          expect(morphTargets.targets, hasLength(1), reason: "instance $instanceIndex lost its morph targets");
