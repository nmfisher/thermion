...
| 2 | Skip overlay passes when empty | **Implemented** | `FFIHighlightOverlayManager._reconcileRenderPlan()` derives target routing and pass activation from the current highlight/output state after every highlight, viewport, or presentation-target change. Both auxiliary views are published together with `RenderManager.setRenderables`; no resources are destroyed or rebuilt. `View.setPresentationRenderTarget` separates platform output ownership from raw Filament target binding, so `FFIView.setRenderTarget` no longer intercepts or bypasses itself. Structural item 8 (skipping allocation of the offscreen main-scene texture entirely) is still open. |
...
**Fix required along the way:** `FilamentApp.capture` rendered every attached view, including inactive ones. While the overlay is suspended the edge view can share the presentation target with the main view, so rendering it after the main pass cleared the captured output. `RenderManager.getRenderPlan()` now returns one immutable snapshot of ordered active/inactive passes. Capture uses that same snapshot in both its primary and WebGL completion frames, while still reading back every attached view for per-pass diagnostics.
...
**Verification:** focused Dart analysis and `flutter analyze` — 0 errors. `view_tests` — 29 passed, including idle-overlay resize and presentation-target replacement coverage. Serial full test suite — 78 passed, 1 pre-existing failure (`input_pipeline_test` toString case, fails on the clean tree too). The default parallel full-suite invocation remains unsupported by the native test harness (`RenderThread` thread-adoption precondition); this is unrelated to the overlay changes.
