...
```bash
git clone https://github.com/nmfisher/thermion && cd thermion/examples/flutter/quickstart
...
Thermion is a toolkit for creating cross-platform 3D PBR applications with Dart and/or Flutter. 

It is mostly a thin wrapper and high-level abstraction over the [Filament engine](https://github.com/google/filament).

You can choose to use the simplified high-level API, or work directly with the lower-level Filament API.

*Thermion is not a game engine*.
...
- Texture Management: It creates, resizes, and destroys the FlutterMetalTextureWrapper used to display the rendered content from the ThermionViewer.
