---
title: Quick Start
description: Render a 3D model with ViewerWidget.
---
...
| `directLight` | `DirectLight?` | `null` | A direct light (sun, point, or spot) to add to the scene |
...
| `onViewerAvailable` | `Future Function(ThermionViewer)?` | `null` | Callback invoked once the viewer is ready |
| `onAssetLoaded` | `Future Function(ThermionViewer, ThermionAsset)?` | `null` | Callback invoked when the asset has finished loading |
| `enableHighlights` | `bool` | `false` | Enable the highlight overlay system for rendering entity outlines |
...
- The widget requires that you have the correct environment setup for Thermion (Flutter stable channel with native assets enabled).
