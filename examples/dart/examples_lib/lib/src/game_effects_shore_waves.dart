...
Future<void> setupShoreWaves(
  ...
  await camera.lookAt(Vector3(0, 4.5, -4.8), focus: Vector3(0, -0.2, 1.2));
  ...
  // Unlit opaque colors are strongly lifted by the headless exposure path;
  // keep the source dark so the rendered beach remains warm tan, not white.
  await sand.setParameterFloat4("sandColor", 0.085, 0.050, 0.022, 1.0);
  ...
  await water.setParameterFloat("waveHeight", 0.24);
  ...
  await water.setParameterFloat("detailStrength", 0.78);

  final surface = await viewer.createGeometry(
    subdividedPlane(
        width: 18.0, depth: 18.0, subdivisionsX: 150, subdivisionsZ: 180),
    materialInstances: [water],
  );
  await surface.setTransform(Matrix4.identity());
